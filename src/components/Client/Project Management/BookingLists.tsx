...
const BookingLists = ({ user }: { user: User[] }) => {
  ...
  return (
    <div className="p-6 space-y-8">
      {user.map((u) => {
        ...
        return (
          <div key={u.name} className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
            {/* Header */}
              ...
              {bookedEvents && bookedEvents.length > 0 ? (
                <div className="space-y-6">
                  {bookedEvents.map((event) => {
                    ...
                    const { upcoming, past } = categorizeBookings(eventBookings);
                    
                    return (
                      <div key={event.id} className="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
                        {/* Event Header */}
                          ...
                          <div className="flex items-start justify-between">
                            ...
                            <div className="text-right ml-4">
                              <div className="text-sm text-gray-500 dark:text-gray-400">
                                {eventBookings.length} booking{eventBookings.length !== 1 ? 's' : ''}
                              </div>
                            </div>
                          </div>
                        </div>
                          ...
                          {upcoming.length > 0 && (
                            <>
                              ...
                              {upcoming.map((booking) => {
                                ...
                                return (
                                  <div key={booking.id} className="px-6 py-4 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                    <div className="flex items-center justify-between">
                                          ...
                                          <div className="flex items-center gap-2">
                                            {booking.additionalInfo && (
                                              <div className="flex items-center gap-1 text-xs text-gray-500 dark:text-gray-400">
                                                ...
                                                <span className="truncate max-w-32" title={booking.additionalInfo}>
                                                  {booking.additionalInfo}
                                                </span>
                                              </div>
                                            )}
                          ...
                          {past.length > 0 && (
                            <>
                              ...
                              {past.map((booking) => {
                                ...
                                return (
                                  <div key={booking.id} className="px-6 py-4 opacity-75">
                                    <div className="flex items-center justify-between">
                                      <div className="flex items-center gap-4 flex-1">
